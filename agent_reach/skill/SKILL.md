...
Twitter 注意：`agent-reach configure twitter-cookies` 保存的 Cookie 会由
`doctor` / `configure` 注入 `twitter status` 子进程的 `TWITTER_AUTH_TOKEN`
和 `TWITTER_CT0`，不会设置当前 Shell。直接运行 `twitter` 前，必须在子进程
环境中显式提供这两个变量，不得在日志或命令回显中暴露值。
