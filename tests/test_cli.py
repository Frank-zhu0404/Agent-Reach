...
class TestCLI:
    ...
    def test_twitter_config_verification_preserves_existing_env_priority(
        ...
        assert calls[0][0] == ["/bin/twitter", "status"]
    ...
    def test_twitter_config_verification_injects_saved_creds_into_child_env(
        self, monkeypatch, capsys
    ):
        ...
        assert "TWITTER_AUTH_TOKEN" not in cli.os.environ
        assert "TWITTER_CT0" not in cli.os.environ
        assert "Twitter access works" in capsys.readouterr().out
